# تشغيل ScoreTime V1.7.2 من الصفر
...
4. يمكنك إدخال رابط API الحقيقي المنتهي بـ`/api/v1` أو ترك الحقل فارغاً.
5. بعد نجاح الـworkflow نزّل Artifact باسم `ScoreTime-V1.7.2-Android`؛ ستجد APK وAAB داخله.

إذا تركت الحقل فارغاً أو وضعت `example.com`، لا يفشل البناء ولا تُعرض بيانات وهمية؛ يفتح التطبيق شاشة ربط Laravel عند أول تشغيل. ألصق فيها رابط HTTPS الحقيقي. يقبل النظام أيضاً الرابط المنسوخ بصيغة `[https://...](https://...)` وينظفه تلقائياً.
...
لا ينشر Workflow بيانات عرض بصمت. عند غياب الرابط ينشر شاشة الإعداد، ويمكن السماح ببيانات العرض يدوياً فقط من خيار `allow_demo_mode`.

لا تحتاج تنفيذ `flutter create` بنفسك. الحزمة ترفع مجلد `mobile-flutter/web` إلى GitHub، والـWorkflow ينفذ تهيئة Web تلقائياً ويحفظ `index.html` و`manifest.json` وأيقونات ScoreTime قبل `flutter build web`.
