...
check("ScoreTime-V1.7.3-Android" in android_workflow, "Android artifact version is stale")
check("android.permission.INTERNET" in android_workflow, "release Android INTERNET permission guard missing")
check("usesCleartextTraffic" in android_workflow, "private-LAN Android HTTP support missing")
...
check("resolve_public_api_url.sh" in android_workflow, "Android Markdown URL resolver missing")
...
app_config = (ROOT / "mobile-flutter/lib/core/config/app_config.dart").read_text(encoding="utf-8")
api_client = (ROOT / "mobile-flutter/lib/core/network/api_client.dart").read_text(encoding="utf-8")
main_dart = (ROOT / "mobile-flutter/lib/main.dart").read_text(encoding="utf-8")
...
check("final bool compact" not in home, "unused MomentumPainter compact parameter remains")
check("if (i == 0) path." not in home and "if(i==0) area." not in home, "unbraced painter if/else remains")
...
check("defaultValue: ''" in app_config, "a fake or local API URL is still compiled by default")
check("normalizeApiUrl" in app_config and "isValidPublicApiUrl" in app_config, "runtime API URL safety is missing")
check("isValidPrivateLanApiUrl" in app_config, "private-LAN API URL guard is missing")
check("enablePreviewMode" in app_config, "explicit preview choice is missing")
check("options.baseUrl = AppConfig.apiBaseUrl" in api_client, "runtime API URL changes do not reach requests")
check("await AppConfig.initialize()" in main_dart and "ApiSetupScreen" in main_dart, "first-launch API setup is not wired")
check((ROOT / "mobile-flutter/lib/features/settings/presentation/api_setup_screen.dart").exists(), "API setup screen missing")


version = (ROOT / "VERSION").read_text(encoding="utf-8").strip()
pubspec = (ROOT / "mobile-flutter/pubspec.yaml").read_text(encoding="utf-8")
check(version == "1.7.3", "root VERSION is not 1.7.3")
check("version: 1.7.3+43" in pubspec, "Flutter build version is not 1.7.3+43")


print("ScoreTime V1.7.3 release validator")
...
print("PASS")
print("Easy Connect, private-LAN guard, explicit preview, Web self-healing, truthfulness, quota gates, six locales and workflows passed.")
