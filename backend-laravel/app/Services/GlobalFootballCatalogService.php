...
use App\Models\{Competition,CompetitionSeason,FootballCountry,FootballMatch,Player,PlayerInjury,PlayerSeasonStat,Standing,Team,Transfer};
...
class GlobalFootballCatalogService {
 ...
 public function syncLeague(int|string $providerLeagueId,int $season,bool $includePlayers=true):array{
  ...
  foreach($this->provider()->fixtures(['league'=>$providerLeagueId,'season'=>$season]) as $raw){$fid=data_get($raw,'fixture.id');$home=Team::where('provider_id',(string)data_get($raw,'teams.home.id'))->first();$away=Team::where('provider_id',(string)data_get($raw,'teams.away.id'))->first();if(!$fid||!$home||!$away)continue;$rawStatus=(string)data_get($raw,'fixture.status.short','NS');FootballMatch::updateOrCreate(['provider_id'=>(string)$fid],['competition_id'=>$competition->id,'home_team_id'=>$home->id,'away_team_id'=>$away->id,'kickoff_at'=>data_get($raw,'fixture.date'),'status'=>FootballStatus::canonical($rawStatus),'minute'=>data_get($raw,'fixture.status.elapsed'),'home_score'=>(int)(data_get($raw,'goals.home')??0),'away_score'=>(int)(data_get($raw,'goals.away')??0),'venue'=>data_get($raw,'fixture.venue.name'),'round'=>data_get($raw,'league.round'),'realtime_state'=>FootballStatus::isLive($rawStatus)?'active':'idle','broadcast_meta'=>['provider'=>$this->provider()->name(),'provider_status'=>$rawStatus],'last_synced_at'=>now()]);$fixtures++;}
 ...
 public function syncTeamTransfers(Team $team):array{
  if(!$team->provider_id)return['transfers'=>0];$count=0;
  foreach($this->provider()->transfers(['team'=>$team->provider_id])as$row){$p=$row['player']??[];if(empty($p['id'])||empty($p['name']))continue;$player=Player::updateOrCreate(['provider_id'=>(string)$p['id']],['team_id'=>$team->id,'name'=>$p['name'],'slug'=>Str::slug($p['name'].'-'.$p['id']),'last_synced_at'=>now()]);foreach(($row['transfers']??[])as$move){$from=$this->externalTeam(data_get($move,'teams.out'));$to=$this->externalTeam(data_get($move,'teams.in'));$date=$move['date']??null;if(!$date)continue;$type=mb_substr((string)($move['type']??'transfer'),0,30);Transfer::updateOrCreate(['player_id'=>$player->id,'from_team_id'=>$from?->id,'to_team_id'=>$to?->id,'transfer_date'=>$date,'type'=>$type],['status'=>'confirmed','source_name'=>$this->provider()->name(),'headline'=>trim($p['name'].' — '.($from?->name_en??'').' → '.($to?->name_en??'')),'last_verified_at'=>now()]);$count++;}}
  return['transfers'=>$count];
 }

 public function syncLeagueInjuries(int|string $providerLeagueId,int $season):array{
  $count=0;foreach($this->provider()->injuries(['league'=>$providerLeagueId,'season'=>$season])as$row){$p=$row['player']??[];if(empty($p['id'])||empty($p['name']))continue;$team=$this->externalTeam($row['team']??[]);$player=Player::updateOrCreate(['provider_id'=>(string)$p['id']],['team_id'=>$team?->id,'name'=>$p['name'],'slug'=>Str::slug($p['name'].'-'.$p['id']),'photo_url'=>$p['photo']??null,'last_synced_at'=>now()]);$reason=mb_substr((string)($p['reason']??'Injury'),0,120);PlayerInjury::updateOrCreate(['player_id'=>$player->id,'type'=>$reason,'status'=>'out'],['team_id'=>$team?->id,'note'=>'Verified by '.$this->provider()->name().' at '.now('UTC')->toIso8601String()]);$count++;}return['injuries'=>$count];
 }

 private function externalTeam(?array $raw):?Team{
  if(empty($raw['id'])||empty($raw['name']))return null;return Team::updateOrCreate(['provider_id'=>(string)$raw['id']],['name_ar'=>$raw['name'],'name_en'=>$raw['name'],'slug'=>Str::slug($raw['name'].'-'.$raw['id']),'logo_url'=>$raw['logo']??null,'last_synced_at'=>now()]);
 }
}
